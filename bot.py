...
if not BOT_TOKEN:
    raise ValueError("BOT_TOKEN missing - set it in Render's environment variables")
if not CHAT_ID:
    raise ValueError("CHAT_ID missing - set it in Render's environment variables")
if not TWELVE_DATA_API_KEY:
    raise ValueError("TWELVE_DATA_API_KEY missing - set it in Render's environment variables")

SYMBOL = "BTC/USD"
NAME = "BTC/USDT"

# BTC prices are in the tens of thousands, whole-dollar precision is enough.
DECIMALS = 1
...
# --- 24h change is fetched separately and less often, since /quote costs
# an extra API credit and we don't need it every 30s ---
CHANGE_REFRESH_EVERY = 20  # roughly every 10 minutes at 30s/cycle
...
def build_status_message():
    ...
    change_txt = str(round(latest["change"], 2)) + "%" if latest["change"] is not None else "..."

    msg = (
        NAME + "\n"
        "Price: " + str(round(latest["price"], DECIMALS)) + "\n"
        "24h Change: " + change_txt + "\n"
...
def build_price_message():
    if latest["price"] is None:
        return "Still building data, give it a moment..."
    change_txt = str(round(latest["change"], 2)) + "%" if latest["change"] is not None else "..."
    return NAME + " price: " + str(round(latest["price"], DECIMALS)) + " (" + change_txt + " 24h)"
...
def get_price():
    """
    Returns (price, error_label). error_label is None on success.
    Uses Twelve Data's lightweight /price endpoint - just the current price,
    no extra fields, to keep API credit usage minimal.
    """
    try:
        r = requests.get(
    ...
    try:
        ...
        if "price" not in data:
            # Twelve Data returns {"code":..., "message":...} on errors like bad API key
            return None, "API error: " + str(data.get("message", data))
...
def get_24h_change():
    """
    Returns (change_pct, error_label). Uses /quote, which is a slightly
    heavier endpoint, so this is only called occasionally rather than
    every cycle.
    """
    try:
        r = requests.get(
    ...
    try:
        ...
        if "percent_change" not in data:
            return None, "API error: " + str(data.get("message", data))
        return float(data["percent_change"]), None
    except (KeyError, ValueError, TypeError) as e:
        return None, "bad response: " + str(e)
...
send(
    "BTC Bot Started! Now tracking BTC/USDT (via Twelve Data BTC/USD). Building data, wait 2 minutes for first signals.\n\n"
    "Commands: /status, /price"
)
print("Bot started - BTC/USDT only, interactive mode", flush=True)
counter = 0
...
while True:
    try:
        ...
        if price is not None:
            ...
            # Refresh 24h change occasionally, not every cycle
            if counter % CHANGE_REFRESH_EVERY == 0:
                change, change_err = get_24h_change()
                if change is not None:
                    latest["change"] = change
                elif change_err:
                    print("24h change fetch failed: " + change_err, flush=True)

            rtxt = str(round(r, 1)) if r is not None else "..."
