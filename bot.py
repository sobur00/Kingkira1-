...
def build_status_message():
    ...
    msg = (
        NAME + "\n"
        "Price: " + str(round(latest["price"], 2)) + "\n"
...
def build_price_message():
    if latest["price"] is None:
        return "Still building data, give it a moment..."
    return NAME + " price: " + str(round(latest["price"], 2)) + " (" + str(round(latest["change"], 2)) + "% 24h)"
...
def get_price():
    try:
        ...
        change = float(r["bitcoin"].get("usd_24h_change", 0))
        return price, change
    except:
        return None, None
...
send(
    "BTC Bot Started! Now tracking BTC/USDT. Building data, wait 2 minutes for first signals.\n\n"
